...
async function assemble(
  ...
  const product: ProductWithRelations = {
    ...
    zerospikeOffer: input.zerospikeOffer ?? null,
  ...
  if (prisma && dataSource() === "database") {
    ...
    const created = await prisma.product.create({
      data: {
        ...
        bulletPoints: built.bulletPoints ?? [],
        specsJson: (built.specs ?? []) as object,
        tags: built.tags ?? [],
        ratingAvg: built.ratingAvg ?? null,
        ratingCount: built.ratingCount ?? null,
        zerospikeOffer: built.zerospikeOffer ?? null,
